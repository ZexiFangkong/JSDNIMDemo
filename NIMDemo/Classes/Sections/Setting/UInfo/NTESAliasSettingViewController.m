...
- (void)onDone:(id)sender{
    ...
    NSDictionary* serverExt = @{
                                @"tagName": self.alias
                                };
    NSError* error;
    NSData* jsonData = [NSJSONSerialization dataWithJSONObject:serverExt options:NSJSONWritingPrettyPrinted error:&error];
    NSString* jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    self.user.ext = jsonString;
